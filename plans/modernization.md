...
**Verdict:** ✅ Cache-busting `?v=<pkg.version>` added via `SnippetOptions.version` in `lib/snippet.ts`. `lib/server.ts` reads `package.json` at startup and passes the version.
...
**Verdict:** ✅ VPN interface filtering added — `getLocalIp()` now skips interfaces matching `/^(tun|utun|tap|vpn)/i`.
...
**Verdict:** ✅ Clone + `onload` swap ported to `handleCssReload` — no FOUC. `pickBestMatch` with `numberOfMatchingSegments` ported for accurate stylesheet targeting. Image reload and `@import` traversal intentionally omitted.
...
Final pass through `legacy/packages/browser-sync/`, `legacy/packages/browser-sync-client/`, and `legacy/packages/browser-sync-ui/` comparing against the new implementation. The items below are gaps we haven't intentionally dropped and haven't yet implemented.

---

### Client-Side Gaps

#### 1. WebSocket Auto-Reconnect ✅ DONE

Extracted to `lib/client/reconnect.ts` with `openReconnecting()` — injectable factory, exponential backoff (1s→30s cap), reset on connect. 11 tests in `lib/client/reconnect.test.ts`.

---

#### 2. Ghost Mode: Pathname Guard ✅ DONE

`GhostMessage` now includes `pathname: string`. `BsClientInfo` tracks `pathname` (default `'/'`). `BsSockets.relay()` updates sender's pathname and only forwards to clients on the same pathname. Tests updated; new pathname-guard test added to `lib/sockets.test.ts`.

---

#### 3. Ghost Mode: Click Label Filtering ✅ DONE

Click listener in `lib/client/index.ts` now skips sends when target is a `<label>` with `htmlFor` pointing to an existing element.

---

#### 4. Ghost Mode: Form Input Echo Suppression ✅ DONE

`handleInput()` in `lib/client/handlers.ts` sets a per-element suppression flag for 800ms. `isInputSuppressed(id)` exported and checked in `input`/`change` listeners before sending ghost messages.

---

#### 5. Scroll: Proportional Coordinates ✅ DONE

`GhostMessage` scroll type now includes `xp`, `yp` (proportional 0–1). Client sends computed proportionals; `handleScroll(x, y, xp, yp)` applies proportional values scaled to recipient's scrollWidth/scrollHeight.

---

### Server-Side Gaps

#### 6. `reloadDelay` Not Wired Up ✅ DONE

Watcher `change` handler in `lib/server.ts` now wraps the broadcast in `setTimeout(doBroadcast, opts.reloadDelay)` when `reloadDelay > 0`.

---

#### 7. `watchEvents` Option Missing ✅ DONE

`watchEvents: string[]` added to `BsOptionsSchema`, `BsOptions`, and `parseOptions` (default `['change', 'add', 'unlink']`). `BsWatcher` filters chokidar events against this list. Passed through from `lib/server.ts`.

---

#### 8. HTTP Control API: Only Reload Implemented ✅ DONE

`POST /__bs/notify` added to `lib/server.ts` — accepts `{ message: string }` JSON body and broadcasts a `notify` message to all connected clients.

---

#### 9. Config File Auto-Loading ✅ DONE

`lib/cli.ts` now dynamically imports `bs-config.js` from `process.cwd()` using `pathToFileURL`. Spreads fileConfig as base options, then overlays explicitly-passed CLI flags. Removed parseArgs defaults from `port` and `log-level` so undefined means "not passed."
...
| Gap | Status |
|---|---|
| WebSocket auto-reconnect | ✅ Done — `lib/client/reconnect.ts`, 11 tests |
| Ghost mode pathname guard | ✅ Done — protocol, sockets, client updated; 2 tests added |
| Form input echo suppression | ✅ Done — `handleInput` sets 800ms suppress flag |
| Click label filtering | ✅ Done — skip `<label htmlFor>` clicks |
| `reloadDelay` wiring | ✅ Done — `setTimeout` wrap in server watcher handler |
| Scroll proportional coords | ✅ Done — `xp`/`yp` in protocol, sent and applied in client |
| `watchEvents` option | ✅ Done — added to schema, watcher, and server |
| HTTP notify endpoint | ✅ Done — `POST /__bs/notify` in server |
| Config file auto-load | ✅ Done — dynamic import in CLI with CLI-flag override |
| Script cache-bust | ✅ Done — `?v=<version>` in snippet via `lib/snippet.ts` |
| VPN IP filtering | ✅ Done — skip `tun/utun/tap/vpn` interfaces in `lib/ip.ts` |
| CSS reload FOUC fix | ✅ Done — clone+onload swap in `handleCssReload` |
| `pickBestMatch` | ✅ Done — segment-based matching in `lib/client/handlers.ts` |
| Delete `.legacy/` | ✅ Done |

All tests, types, and lint passing.
